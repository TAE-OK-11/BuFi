...
actor AppDatabase {
    ...
    func loadTrackIntelligence(scope: String) async -> [String: LyricSignature] {
        guard let pool = await databasePool() else { return [:] }
        do {
            return try await pool.read { db in
                let rows = try Row.fetchAll(
                    db,
                    sql: """
                    SELECT * FROM track_intelligence WHERE account_scope = ?
                    """,
                    arguments: [scope]
                )
                return Dictionary(uniqueKeysWithValues: rows.compactMap { row in
                    Self.lyricSignature(from: row).map { ($0.songID, $0) }
                })
            }
        } catch {
            return [:]
    ...
    @discardableResult
    func saveTrackIntelligence(
        _ signature: LyricSignature,
        scope: String
    ) async -> Bool {
        guard let pool = await databasePool() else { return false }
        do {
            try await pool.write { db in
                try db.execute(
                    ...
                    ON CONFLICT(account_scope, song_id) DO UPDATE SET
                        ...
                        sound_source = excluded.sound_source,
                        sound_labels_json = excluded.sound_labels_json,
                    ...
                    arguments: [
                        scope,
                        signature.songID,
                        signature.lyricsHash,
                        signature.audioRevision,
                        Self.encodeJSONList(signature.moods),
                        Self.encodeJSONList(signature.themes),
                        signature.energy,
                        signature.valence,
                        signature.source,
                        signature.soundSource,
                        Self.encodeJSONList(signature.soundLabels),
    ...
    private static var migrator: DatabaseMigrator {
        ...
        migrator.registerMigration("track-intelligence-v7") { db in
            try db.execute(sql: """
                CREATE TABLE track_intelligence (
                    account_scope TEXT NOT NULL,
                    song_id TEXT NOT NULL,
                    lyrics_hash TEXT NOT NULL DEFAULT '',
                    audio_revision TEXT NOT NULL DEFAULT '',
                    moods_json TEXT NOT NULL DEFAULT '[]',
                    themes_json TEXT NOT NULL DEFAULT '[]',
                    energy DOUBLE NOT NULL DEFAULT 0.5,
                    valence DOUBLE NOT NULL DEFAULT 0.5,
                    lyric_source TEXT NOT NULL DEFAULT '',
                    sound_source TEXT NOT NULL DEFAULT '',
                    sound_labels_json TEXT NOT NULL DEFAULT '[]',
                    lexical_embedding BLOB NOT NULL,
                    sentence_embedding BLOB NOT NULL,
                    sound_embedding BLOB NOT NULL,
                    updated_at DOUBLE NOT NULL,
                    PRIMARY KEY (account_scope, song_id)
                ) WITHOUT ROWID;
                CREATE INDEX track_intelligence_lyrics
    ...
    private static func lyricSignature(from row: Row) -> LyricSignature? {
        let songID: String = row["song_id"]
        guard !songID.isEmpty else { return nil }
        let lyricsHash: String = row["lyrics_hash"]
        let moodsJSON: String = row["moods_json"]
        let themesJSON: String = row["themes_json"]
        let energy: Double = row["energy"]
        let valence: Double = row["valence"]
        let lyricSource: String = row["lyric_source"]
        let soundSource: String = row["sound_source"]
        let soundLabelsJSON: String = row["sound_labels_json"]
        let lexical: Data = row["lexical_embedding"]
        ...
        return LyricSignature(
            ...
            moods: decodeJSONList(moodsJSON),
            themes: decodeJSONList(themesJSON),
            energy: energy,
            valence: valence,
            embedding: decodeFloats(lexical),
            source: lyricSource,
            sentenceEmbedding: decodeFloats(sentence),
    ...
    private static func decodeJSONList(_ raw: String?) -> [String] {
