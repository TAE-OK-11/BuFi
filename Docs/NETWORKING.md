...
- A new play command starts AVAudioSession, URL/asset resolution, current-track
  lyrics, canonical metadata, and current artwork in the same scheduling turn.
  Music, lyrics, and cover art therefore load concurrently. Only optional
  next-track and offline work waits for an additional stable-playback window.
