...
controls_start = detail.read_text().index("    private var controls: some View {")
controls_end = detail.read_text().index("    private var artistAbout: some View {", controls_start)
...
background_start = detail.read_text().index("    private var background: some View {")
background_end = detail.read_text().index("    private var isArtist: Bool {", background_start)
text = detail.read_text()
new_background_and_helpers = '''    private var background: some View {
        ZStack {
            LinearGradient(
                colors: [
...
'''
detail.write_text(text[:background_start] + new_background_and_helpers + text[background_end:])
...
for path in (detail, settings, project):
    print(path, "updated", len(path.read_text().splitlines()), "lines")
